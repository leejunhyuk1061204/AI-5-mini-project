...
const LiveSttPage: React.FC = () => {
  ...
  const [currentMeetingId, setCurrentMeetingId] = useState<number | null>(meetingIdParam ? Number(meetingIdParam) : null);
  ...
  // stop이 "사용자 버튼 클릭"으로 온 건지 표시
  const stopByUserClickRef = useRef(false);

  // =========================
  // Auto-save temp data
  // =========================
  useEffect(() => {
    const dataToSave = { transcripts, summary };
  ...
  useEffect(() => {
    if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        if (final) setTranscripts(prev => [...prev, final]);
    ...
    return () => {
      trace('useEffect cleanup running (component unmount?)');

      if (recognitionRef.current) {
        trace('cleanup: recognition.stop()');
        recognitionRef.current.stop();
      }

      if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
        trace('cleanup: mediaRecorder.stop()');
        mediaRecorderRef.current.stop();
      }

      if (mediaStreamRef.current) {
        trace('cleanup: stop mediaStream tracks');
        mediaStreamRef.current.getTracks().forEach(track => track.stop());
      }

      if (wsRef.current) {
        trace('cleanup: ws.close()');
        wsRef.current.close();
        wsRef.current = null;
      }
    };
  ...
  const connectWebSocket = (meetingId: number) => {
    return new Promise<WebSocket>((resolve, reject) => {
      ...
      ws.onclose = (e) => {
        ...
        trace(`ws.onclose fired (code=${(e as CloseEvent).code}) stopByUserClick=${stopByUserClickRef.current}`);
      };

      ws.onerror = (error) => {
        console.error('WebSocket 오류:', error);
        trace('ws.onerror fired');
        reject(error);
      };
    });
  };
  ...
  const disconnectWebSocket = () => {
    if (wsRef.current) {
      trace(`disconnectWebSocket() called. stopByUserClick=${stopByUserClickRef.current}`);
      wsRef.current.close();
      wsRef.current = null;
  ...
  const startMediaRecorder = async (meetingId: number) => {
    try {
      trace(`startMediaRecorder(meetingId=${meetingId})`);
      ...
      if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
        ...
        trace('startMediaRecorder aborted: ws not OPEN');
      ...
      mediaRecorder.ondataavailable = (event) => {
        if (event.data.size > 0) {
          ...
          if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            ...
            console.warn('WS 아직 OPEN 아님. 청크 전송 스킵', { wsState });
            trace(`chunk skipped (wsState=${wsState})`);
          }
        }
      };

      mediaRecorder.onstop = () => {
        trace(`mediaRecorder.onstop fired. stopByUserClick=${stopByUserClickRef.current}`);
  ...
  const stopMediaRecorder = () => {
    trace(`stopMediaRecorder() called. stopByUserClick=${stopByUserClickRef.current}`);
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      mediaRecorderRef.current.stop();
  ...
  const toggleListening = async () => {
    trace(`toggleListening() called. isListening=${isListening}`);

    if (isListening) {
      // ✅ 사용자가 stop 버튼을 눌렀을 때 표시
      stopByUserClickRef.current = true;
      trace('User clicked STOP (isListening was true)');

      recognitionRef.current?.stop();
      stopMediaRecorder();
    } else {
      stopByUserClickRef.current = false;
      trace('User clicked START (isListening was false)');
  ...
  const handleSummarize = () => {
    ...
    setTimeout(() => {
      const mockSummary =
        '이번 회의에서는 주요 프로젝트 일정에 대한 논의가 있었습니다. 팀원들은 각자의 진행 상황을 공유했고, 다음 주까지 프로토타입을 완성하기로 합의했습니다. 또한, 디자인 시안에 대한 피드백을 수집하여 수요일까지 수정안을 제출하기로 결정했습니다.';
            ...
            <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0 no-scrollbar">
              <button
                ...
                className={`px-3 py-2 md:px-4 text-sm font-bold text-white rounded-lg transition-all shadow-sm flex items-center gap-2 whitespace-nowrap shrink-0
                  ${isSummarizing || isListening ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700'}
                `}
              >
                {isSummarizing ? (
                  <>
          ...
          <div className="flex flex-col items-center gap-4 pt-6 pb-2 shrink-0">
            ...
            <button
              ...
              className={`
                ...
                ${isListening ? 'bg-red-500 hover:bg-red-600 shadow-red-500/30' : 'bg-[#135bec] hover:bg-blue-700 shadow-blue-500/30'}
              `}
            >
              <span className="material-symbols-outlined text-white text-[32px]">{isListening ? 'stop' : 'mic'}</span>
